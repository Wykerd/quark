...
X509_STORE* qrk_sec_ctx_new_root_cert_store (qrk_malloc_ctx_t *ctx, int use_openssl_default_store) {
    ...
    if (use_openssl_default_store) {
        ...
        if (!root_certs_loaded) {
            for (size_t i = 0; i < root_certs_len; i++) {
                BIO* inbio = qrk_crypto_bio_new_from_buf_fixed(ctx, root_certs[i], strlen(root_certs[i]));
                if (!inbio)
                {
                ...
                X509* x509 = PEM_read_bio_X509(inbio, NULL, qrk_crypto_no_password_cb, NULL);
                BIO_free(inbio);
...
unsigned long qrk_sec_ctx_set_key (qrk_secure_ctx_t *ctx, BIO *key, const qrk_rbuf_t *password)
{
    ERR_clear_error();
...
unsigned long qrk_sec_ctx_set_cert (qrk_secure_ctx_t *ctx, BIO *cert) {
    int r = qrk_sec_ctx_ssl_ctx_use_certificate_chain_bio(ctx->ssl_ctx, cert);
    if (r)
        return r;
...
unsigned long qrk_sec_ctx_add_cacert (qrk_secure_ctx_t *ctx, BIO *bio) {
...
unsigned long qrk_sec_ctx_add_crl (qrk_secure_ctx_t *ctx, BIO *bio) {
    X509_CRL *crl = PEM_read_bio_X509_CRL(bio, NULL, qrk_crypto_no_password_cb, NULL);
...
unsigned long qrk_sec_ctx_set_cipher_suites (qrk_secure_ctx_t *ctx, const char* ciphers) {
    ERR_clear_error();
...
unsigned long qrk_sec_ctx_set_ciphers (qrk_secure_ctx_t *ctx, const char* ciphers) {
    ERR_clear_error();
...
unsigned long qrk_sec_ctx_set_ecdh_curve (qrk_secure_ctx_t *ctx, const char* curve) {
...
unsigned long qrk_sec_ctx_set_sigalgs (qrk_secure_ctx_t *ctx, const char* sigalgs) {
    ERR_clear_error();
